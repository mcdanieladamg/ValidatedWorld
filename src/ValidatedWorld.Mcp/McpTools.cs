...
internal sealed class McpTools(McpProjectService projects)
{
    ...
    [McpServerTool(UseStructuredContent = true, Destructive = false, OpenWorld = false), Description("Initializes a new local ValidatedWorld project containing only its purpose node, then selects it for this MCP session. The destination is never overwritten.")]
    ...
    [McpServerTool(UseStructuredContent = true, Destructive = false, OpenWorld = false), Description("Adds or replaces one complete node in the current proposal. This is a convenience wrapper over the same bounded application batch operation.")]
    ...
    [McpServerTool(UseStructuredContent = true, Destructive = false, OpenWorld = false), Description("Adds or replaces one complete edge in the current proposal. This is a convenience wrapper over the same bounded application batch operation.")]
    ...
    [McpServerTool(UseStructuredContent = true, Destructive = false, OpenWorld = false), Description("Removes one node or edge explicitly from the current proposal. Incident edges are never silently cascaded.")]
    ...
    [McpServerTool(UseStructuredContent = true, Destructive = false, OpenWorld = false), Description("Discards the current unresolved in-memory proposal without writing it.")]
    ...
    [McpServerTool(UseStructuredContent = true, ReadOnly = true, Destructive = false, OpenWorld = false, Idempotent = true), Description("Finds a bounded directed review-dependency path between two selected-project nodes.")]
